...
import spinal.core.{IntToBuilder, TimeNumber}
...
class UnwrapFloatTest extends AnyFunSuiteLike {
  ...
  def testStreamModule(
    ...
    val result = data.map(frame => Array.fill(frame.length)(0.0f)).toArray
    var pokeRowId, pokeColId, peekRowId, peekColId = 0

    Config.sim.compile(UnwrapFloat()).doSim { dut =>
      // driver thread
      fork {
      ...
      fork { // monitor thread
        ...
        val monitor = StreamMonitor(dut.dataOut, dut.clockDomain) { payload =>
          result(peekRowId)(peekColId) = payload.fragment.toFloat
          peekColId += 1
  ...
  test("test fixed pattern") {
    ...
    val raw = Seq(1.4384286318379038, -2.339298106472155, -0.6321902666025982, 0.9977931740489749, 1.4728361001182702,
      1.540549435381723, -0.5233026416176028, -1.0962312335998163, 0.44271091296475973, 1.6115827435684462).map(
      _.toFloat
    )
