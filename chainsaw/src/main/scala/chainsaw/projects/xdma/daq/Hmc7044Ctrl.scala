...
/** write/read HMC7044 registers through AXI4-Lite interface
  */
case class Hmc7044Ctrl(dividerRatio: Int, useInOut: Boolean = true) extends Component {
  ...
  // SPI interface
  val sclk = out Bool ()
  val slen = out Bool () // enable, active-low
  val sdi = Bool()
  val sdo = Bool()
  ...
  val ctrlFsm = new StateMachine {
    ...
    GET_WRITE_ADDR.whenIsActive { // get address from AXI4-Lite interface
      instructionReg(23 downto 21) := B"000"
      when(user.aw.valid && !awDone) { // wait for user.aw.valid
        instructionReg(20 downto 8) := user.aw.payload.addr.takeLow(13)
        user.aw.ready.set()
        awDone.set()
      }
    }
    ...
    GET_READ_ADDR.whenIsActive {
      instructionReg(23 downto 21) := B"100"
      when(user.ar.valid && !arDone) { // wait for user.ar.valid
        instructionReg(20 downto 8) := user.ar.payload.addr.takeLow(13)
        user.ar.ready.set()
        arDone.set()
      }
    }
