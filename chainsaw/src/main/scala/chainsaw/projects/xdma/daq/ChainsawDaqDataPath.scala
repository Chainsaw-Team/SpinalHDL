  ...
  val controlClockingArea = new ClockingArea(controlClockDomain) {
    ...
    val testMode =
      datapathControlReg.field(
        ...
        RW,
        1,
        "incrementing number will be generated by datapath when test mode turned on"
      )
    ...
    val pulsePeriod = pulsePeriodReg.field(UInt(32 bits), RW, 100)
    val pulseLength = pulseLengthReg.field(UInt(32 bits), RW, 50)
...
object ChainsawDaqDataPath {
  def main(args: Array[String]): Unit = {
    SpinalSimConfig().withWave.compile(ChainsawDaqDataPath()).doSim { dut =>
      ...
      dut.dataClockDomain.waitSampling(5000)
    }
  }
}
