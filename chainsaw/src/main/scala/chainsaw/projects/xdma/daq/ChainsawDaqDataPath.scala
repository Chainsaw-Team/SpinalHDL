...
case class ChainsawDaqDataPath() extends Component {
  ...
  val controlClockingArea = new ClockingArea(controlClockDomain) {
    ...
    val channelSelection = datapathControlReg.field(Bool(), RW, "select channel 0/1 as data source")
    val xyEnabled = datapathControlReg.field(Bool(), RW, "select whether using x+y or x/y only for demodulation")
...
//    }
    ...
    daqDemodulator.channelSelection := getControlData(channelSelection)
    daqDemodulator.xyEnabled := getControlData(xyEnabled)
