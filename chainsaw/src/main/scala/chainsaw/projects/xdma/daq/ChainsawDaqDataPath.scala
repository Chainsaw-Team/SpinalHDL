...
case class ChainsawDaqDataPath() extends Component {
  ...
  val controlClockingArea = new ClockingArea(controlClockDomain) {
    ...
    val demodulationEnabled = datapathControlReg.field(Bool(), RW, "when enabled, demodulated phase, instead of X & Y raw data will be transferred to DDR")
...
//    userBusIf.accept(PythonHeaderGenerator("UserRegisterSpace", "AP"))
  ...
  val dataClockingArea = new ClockingArea(dataClockDomain) {
    // datapath parameters
    val periodCounterWidth = 28 // FIXME: will this lead to a bad critical path?
    val packetIdWidth = 16

    println(
      f"counter width = $periodCounterWidth, " +
        f"max period = ${(1 << periodCounterWidth) * 4} ns, " +
        f"sampling rate lower bound = ${1.0 / ((1 << periodCounterWidth) * 4).toDouble * 1e9} Hz"
    )
    ...
    streamRaw.fragment(0) := channel0Segments(2)
    streamRaw.fragment(1) := channel0Segments(0)
    streamRaw.fragment(2) := channel1Segments(2)
    streamRaw.fragment(3) := channel1Segments(0)
    ...
    // TODO: using test data when testMode on

    // header insertion & buffering
    val header = B(0, (64 - packetIdWidth) bits) ## counterForPacket
    val streamBuffered = streamDemodulated.queue(1024) // buffer between free-running & standard stream interface
