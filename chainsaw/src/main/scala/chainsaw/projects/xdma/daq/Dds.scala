...
/**
 * @param clockFreq
 * @param amplitudePrecision determine the output resolution of DDS
 * @param phasePrecision determine the frequency resolution of DDS
 * @param lutAddrWidth determine the depth of LUT
 * @param channelCount number of time-multiplexing channels
 * @param useDriver
 */
case class DdsConfig(
    clockFreq: HertzNumber,
    amplitudePrecision: Int,
    phasePrecision: Int,
    lutAddrWidth: Int,
    channelCount: Int,
  ...
  def synth(targetFreq: HertzNumber): Int = {
    ...
    val actualIndices = (0 until 1 << 10).map(i => ((i * phaseIncrement) % phaseDepth) >> phaseTruncationWidth)
    val actualLutContent = actualIndices.map(lutContent)
...
case class Dds(ddsConfig: DdsConfig) extends Module {
  ...
  // TODO: mapping optimization

  import ddsConfig._

//  val driverIn = useDriver generate slave Stream Vec(SInt(amplitudePrecision bits), channelCount)
  val ddsRom = Mem(lutContent.grouped(channelCount).map(vec => Vec(vec.map(S(_, amplitudePrecision bits)))).toSeq)
  val lutAddrCounter = CounterFreeRun(1 << lutDepth)
  val dataOut = master Stream Vec(SInt(amplitudePrecision bits), channelCount)

}

object DdsConfig extends App {
  val ddsConfig = DdsConfig(500 MHz, 16, 16, 16, 2)
